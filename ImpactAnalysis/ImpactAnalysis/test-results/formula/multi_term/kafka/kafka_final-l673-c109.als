...
fact ConsumerGroup_facts {
  ...
  all cg : ConsumerGroup {
    ...
    all p : cg.subscribedTo.partitions | one cg.offsets[p] and cg.offsets[p] =< p.leader.events.lastIdx
...
pred consumerCanReadEventFromPartition[k: Kafka, c : Consumer, e: KafkaEvent, p : TopicPartition] {
  ...
  // The Event e must be present at the index that is saved for the partition in consumer's group
  // @cg: ConsumerGroup containing the consumer
...
pred canProducerPushEvent[k : Kafka, prod : Producer, e: KafkaEvent, p : TopicPartition] {
  ...
  // Producer prod should be configured in the cluster
  prod in k.producers
...
/**
 * A predicate used to preserves variables unaffected by brokerRecover
 */
...
pred brokerRecoverFrame[recover_partitions : TopicPartition] {
  ...
  all pre_existing_replicas : dom[events.KafkaEvent] | pre_existing_replicas.events' = pre_existing_replicas.
