...
/**
   * Fact that indicates a product can have at most one ProductAd.
  */
...
fun sharedLikes[m1, m2: Pet]: Int {
  #(m1.likes & m2.likes) // & -> intersection between like sets
}
...
fun productVerifiedByVet[prod: Product]: Int {
  ...
  // For simplicity, a product is verified if it has been reviewed by at least four veterinarians
   #{ v: Veterinarian | prod in v.verifies } > 3 
   implies 1 
   else 0
...
fact eventSuspended{
  all e: Event | {
    (daysRemaining[e.eventDate] =< 7 and #{e.participants}=<6) implies #(e.
  } 
}
