...
fact CausalChain {
	...
	all from, to : Component, t : Time - last |
		from -> to in causes.t iff {
			...
			// there's an intermediate component "cc" such that "from" causes cc and "cc" invokes "to"
			(some cc : Component | from -> cc in causes.t and invoke[t, t.next, cc, to]) or
...
pred grantPermissions[app : Application, t : Time] {
	app.grantedPerms.t.name = app.usesPerms
	app.grantedPerms.t in Device.customPerms.t + Device.builtinPerms
}

// The app installation is described by this operation:
pred install[t, tt: Time, app: Application] {
	app not in Device.apps.t
  	Device.apps.tt = Device.apps.t + app
	grantPermissions[app, tt]
	Device.customPerms.tt = Device.customPerms.t + newCustomPerms[t,app]
	// no changes to other apps
	all a : Application - app | a.grantedPerms.tt = a.grantedPerms.t
}

pred install_fixed[t, tt : Time, app : Application] {
	app not in Device.apps.t
  	Device.apps.tt = Device.apps.t + app
	grantPermissions[app, tt]
	...
	Device.customPerms.tt = Device.customPerms.t + app.declaredPerms
	all a : Application - app | a.grantedPerms.tt = a.grantedPerms.t
}
...
pred uninstall[t, tt: Time, app: Application] {
...
pred uninstall_fixed[t, tt: Time, app: Application] {
	app in Device.apps.t // precondition
	Device.apps.tt = Device.apps.t - app
	Device.customPerms.tt = Device.customPerms.t - app.declaredPerms
	all a : Application - app | a.grantedPerms.tt = a.grantedPerms.t - app.declaredPerms
}
...
pred traces {
	...
	all t: Time-last | let tt = t.next |
		some app: Application, c1, c2: Component, n : Name, u : URI|
			install [t, tt, app]
			or uninstall [t, tt, app]
			or invoke[t, tt, c1, c2]
			or grantURIPermission[t, tt, c1, n, u]
}
...
pred traces_fixed {
	...
	all t: Time-last | let tt = t.next |
		some app: Application, c1, c2: Component, n : Name, u : URI |
			install_fixed [t, tt, app]
			or uninstall_fixed [t, tt, app]
			or invoke[t, tt, c1, c2]
			or grantURIPermission[t, tt, c1, n, u]
}
...
pred noChanges[t, tt: Time] {
	Device.customPerms.tt = Device.customPerms.t
	Device.apps.tt = Device.apps.t
	all a : Application | a.grantedPerms.tt = a.grantedPerms.t
}

// Operation for a content provider to grant an URI permission to another application
pred grantURIPermission[t, tt : Time, granter: ContentProvider, grantee: AppName, u: URI] {
	let granteeApp = findAppByName[grantee, t] {
		...
		some p : URIPermission {
			// postcondition
			p.uri = u
			granteeApp.grantedPerms.tt = granteeApp.grantedPerms.t + p
			all a : Application - granteeApp | a.grantedPerms.tt = a.grantedPerms.t
		}
...
pred invoke[t, tt : Time, caller, callee: Component]{
...
/** The following functions are for visualization only
**/
fun called : Component -> Component -> Time {
	{c1, c2 : Component, t : Time |
		invoke[t, t.next, c1, c2] and t not in last and c1 != c2}
}

fun invokes : Component -> Component -> Time {
	{c1, c2 : Component, t : Time |
		invoke[t, t.next, c1, c2] and not authorized[c1, c2, t] and t not in last and c1 != c2}
}

fun uriGranted : ContentProvider -> URI -> Application -> Time {
	{p : ContentProvider, u : URI, a : Application, t : Time |
		grantURIPermission[t, t.next, p, a.name, u]
	}
}

fun install : Application -> Time {
	{a : Application, t : Time | install[t, t.next, a] and t not in last }
}

fun uninstall : Application -> Time {
	{a : Application, t : Time | uninstall[t, t.next, a] and t not in last }
}
