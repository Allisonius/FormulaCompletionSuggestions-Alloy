...
fun fr : Read->Write {							
  ~rf.co + ((Read - (Write.rf)) <:  ((address.map).~(address.map)) :> Write)		
}
...
fact { all e, ee : Event | all l, ll : Location | e->l->ee->ll in sub_uhb => ( e->l in NodeRel and ee->ll in NodeRel ) }
...
// no iden in uhb 
fact { all e, ee : Event | all l, ll : Location | e->ee in iden and l->ll in iden => not e->l->ee->ll in sub_uhb  }

// node mapping
fact { all e : Event | all l : Location  | e->l in NodeRel => one n : Node | n.event = e and n.loc = l }
fact { all n : Node | n.event->n.loc in NodeRel }
fact { all n, nn : Node | n->nn in uhb <=> n.event->n.loc->nn.event->nn.loc in sub_uhb }

// uhb_intra only relates the same event to different locations
fact 	{ all e, ee : Event 	| all l, ll : Location |  EdgeExists[e, l, ee, ll, uhb_intra] => SameEvent[e, ee] }

// uhb_inter only relates different events on the same core
fact { all e, ee : Event 	| all l, ll : Location | EdgeExists[e, l, ee, ll, uhb_inter] => not SameEvent[e, ee] }
fact { all e, ee : Event 	| all l, ll : Location | EdgeExists[e, l, ee, ll, uhb_inter] => SameThread[e, ee] }
...
pred total[rel: Event->Event, bag: Event] {
  all disj e, ee: bag | e->ee in rel + ~rel	
  acyclic[rel]											
}
pred u_irreflexive[node_rel: Event->Location->Event->Location]       {
	no node_rel or (all e, ee: Event | all l, ll: Location | e->l->ee->ll in node_rel => not ( (e->ee) in iden and (l->ll) in iden ))
}
...
pred EdgeExists[e: Event, l: Location, ee: Event, ll: Location, node_rel: Event->Location->Event->Location] 	{ e->l->ee->ll in node_rel }
...
pred SameProcess[e: Event, ee: Event] { e->ee in process.~process }
pred SameCore[e: Event, ee: Event] { e->ee in core.~core }
pred SameThread[e: Event, ee: Event] { e->ee in ^po + ^~po }
pred SameLocation[l: Location, ll: Location] { l->ll in iden }
pred SameEvent[e: Event, ee: Event] { e->ee in iden }
pred ProgramOrder[e: Event, ee: Event] { e->ee in ^po }
...
pred SameIndexL1 [e: Event, ee: Event] { (e.address).indexL1 = (ee.address).indexL1 }
pred NumProcessThreads[i: Int, P: Process] { #((Event - (Event.po)) & process.P)=i }
pred NumThreads[i: Int] { #(Event - (Event.po))=i }

pred SamePhysicalAddress[e: Event, ee: Event] {
    e->ee in (address.map).~(address.map) or
	e->ee in (flush_addr.map).~(address.map) or
	e->ee in (address.map).~(flush_addr.map) or
	e->ee in (flush_addr.map).~(flush_addr.map)
} 

pred SameVirtualAddress[e: Event, ee: Event] { 
    e->ee in address.~address or
	e->ee in flush_addr.~address or
	e->ee in address.~flush_addr or
    e->ee in flush_addr.~flush_addr
}

pred SameSourcingWrite[r: Read, rr: Read] {
	not SameEvent[r, rr] and (
		DataFromInitialStateAtPA[r] and DataFromInitialStateAtPA[rr] or	// sourced by the same initial value
    	r->rr in ~rf.rf 
	)
}