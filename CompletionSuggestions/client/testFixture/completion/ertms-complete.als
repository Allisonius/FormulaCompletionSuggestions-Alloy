...
fun start_shadow_timer_A[t:Time] : set TTD {
	{ ttd : TTD | some tt: t.*T/prev {
			ttd in occupied[tt.prev]
			ttd not in occupied[tt]
			ttd.end.state.(tt.prev) = Ambiguous
		}
	}
}
...
fun start_shadow_timer_B[t:Time] : set TTD {
	{ ttd : TTD | some tt:t.*T/prev {
			ttd.end.state.tt = Unknown
			ttd.end.state.(tt.prev) = Ambiguous
			some tr:positioned[tt.prev,ttd]-positioned[tt,ttd] | tr not in disintegrated[tt.prev]
...
fun start_integrity_loss_ptimer[t:Time] : set VSS {
	{ vss : VSS | some tt:t.*T/prev {
			vss in (state.(tt.prev)).Occupied
			some tr2 : knownTrain[tt,vss] |
				(tr2 in integrity_timer.tt && tr2 not in integrity_timer.(tt.T/prev))
		}
	}
}
...
fun start_ghost_ptimer[t:Time] : set TTD {
	{ ttd : TTD | some tt:t.*T/prev {
			ttd in occupied[tt]
			ttd not in occupied[tt.prev] && some tt.T/prev
			(no positioned[tt,ttd] || no MAs[tt,ttd])
		}
	}
}
...
pred states[t,tt:Time,vss:VSS] {
	vss.state.tt = (
		n01[t,tt,vss] 	=> 	Unknown else
		n02[t,tt,vss] 	=> 	Occupied else 	// priority over n03
		n03[t,tt,vss] 	=> 	Ambiguous else
		n04[t,tt,vss] 	=> 	Free else	 	// priority over n05 and n12
		n12[t,tt,vss] 	=> 	Occupied else	// priority over n05
		n05[t,tt,vss] 	=> 	Ambiguous else
		n06[t,tt,vss] 	=> 	Free else
		n07[t,tt,vss] 	=> 	Unknown else	// priority over n08
		n08[t,tt,vss] 	=> 	Ambiguous else
		n09[t,tt,vss] 	=> 	Free else		// priority over n10
		n10[t,tt,vss] 	=> 	Unknown else
		n11[t,tt,vss] 	=> 	Occupied else
						vss.state.t
	)
}

// VSS state transition #1 from Free to Unknown
pred n01 [t,tt:Time,v:VSS] {
	v.state.t = Free
	// TTD is occupied, common to all #1
	parent[v] in occupied[tt]
	n01A[tt,v] || n01B[tt,v] || n01C[tt,v] || n01D[tt,v] || n01E[tt,v] || n01F[tt,v]
}
...
pred n04 [t,tt:Time,v:VSS] {
	v.state.t = Unknown
	n04A[tt,v] || n04B[tt,v] || n04C[tt,v]
}
...
pred n08 [t,tt:Time,v:VSS] {
	v.state.t = Occupied
	n08A[tt,v] || n08B[tt,v] || n08C[tt,v]
}
...
pred MAs[t,tt:Time] {
	...
	all tr:connected.t | (tr.MA.tt = tr.MA.t || (knownFront[t,tr].*next&tr.MA.tt.*V/prev).(state.t) = Free || OSMA[tt,tr])
	// if a train is connected, do nothing or remove MA (assign to current known position)
	all tr:(Train-connected.t)+mute_timer.t | (tr.MA.tt = tr.MA.t || tr.MA.tt = knownFront[t,tr])
}
...
pred move [t,tt:Time,tr:Train] {
	// possible effective train movement
	tr.pos_front.tt in tr.pos_front.t + (tr.pos_front.t).V/next
	tr.pos_rear.tt in tr.pos_front.tt + (tr.pos_front.tt).prev
	tr.pos_rear.tt in tr.pos_rear.t + (tr.pos_rear.t).V/next

	// posibility to report PTD
	{
		t in tr.connected
		tt in tr.report_rear => tt in tr.report_front
	} || {
		tt not in tr.report_front
		tt not in tr.report_rear
	}

	// if connected, follow MA
	t in tr.connected => tr.pos_front.tt in MAs[tt,tr]

	// frame condition
	t in tr.connected iff tt in tr.connected
}

// start of mission action
pred som[t,tt:Time,tr:Train] {
	tr not in connected.t
	connected.tt = connected.t + tr

	report_rear.tt = report_rear.t + tr
	report_front.tt = report_front.t + tr

	// frame conditions
	pos_front.tt = pos_front.t
	pos_rear.tt = pos_rear.t
}

// end of mission action
pred eom[t,tt:Time,tr:Train] {
	tr in connected.t
	connected.tt = connected.t - tr
...
pred integral[t:Time,tr,trr:Train] {
	all t0 : t.*T/prev {
		trr.MA.t0 = tr.MA.t0
		trr.pos_front.t0 = tr.pos_front.t0
		trr.pos_rear.t0 = tr.pos_rear.t0
		t0 in trr.report_front iff t0 in tr.report_front
		t0 in trr.report_rear iff t0 in tr.report_rear
	}
}

// encodes the breaking up of an two-carriage train into two.
// front one reports lost integrity, rear one stays disconnecetd.
pred split [t,tt:Time,tr,trr:Train] {
	tr != trr
	integral[t,tr,trr]
	tt not in trr.(report_rear+report_front)
	tr.pos_rear.tt in tr.pos_front.t + tr.pos_rear.t
	trr.pos_front.tt = trr.pos_front.t
	trr.pos_rear.tt = trr.pos_rear.t
	pos_rear.tt - (tr+trr) -> VSS = pos_rear.t - (tr+trr) -> VSS
	pos_front.tt - trr -> VSS = pos_front.t - trr -> VSS
	report_rear.tt = report_rear.t - (trr + tr)
	report_front.tt = report_front.t - trr
	t in trr.connected
	tt not in trr.connected
}
...
fact trace {
	all t:Time,tt:t.next {
		timers[t]
		MAs[t,tt]
		all v:VSS | states[t,tt,v]
		(all tr:Train | move[t,tt,tr]) || (some tr,trr:Train | split[t,tt,tr,trr] || som[t,tt,tr] || eom[t,tt,tr])
	}
	timers[last]
}
...
pred init[t:Time] {
	...
	all tr:Train | tr.MA.t in tr.pos_front.t.*V/next &&
		all trr : Train-tr | no MAs[t,tr] & MAs[t,trr]
}
