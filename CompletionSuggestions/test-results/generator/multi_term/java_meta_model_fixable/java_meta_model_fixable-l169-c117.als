...
// calls a method declared in the class
    all ci: ConstructorMethodInvocation | some c:Class | ci.idClass = c.id &&(some m:Method | m in c.methods && m.id = ci.idMethod)  
...
fact {
    ...
    all ci: ConstructorFieldInvocation | some c:Class | ci.idClass2 = c.id &&(some f:Field | f in c.fields && f.id = ci.idField)
...
pred noCalltoUndefinedMethod {
  all mi:MethodInvocation | some c1,c2: Class, m1:c1.methods, m2:c2.methods |
      (#mi.q = 0 || mi.q = this_) =>  mi in m1.b && mi.id = m2.id && ((c1 = c2) || ((c2 in c1.^extend) && (m2.acc != private_)))
...
pred noSuperCallToNotInheritedMethod {
   all mi:MethodInvocation | some c1,c2: Class, m1:c1.methods, m2:c2.methods |
       mi.q = super_ => 
            mi in m1.b && mi.id = m2.id && c2 in c1.^extend && (m2.acc != private_)
...
pred noSuperCallToNotInheritedField {
  all fi:FieldInvocation | 
      ...
      some disj c1,c2: Class, m1:c1.methods, f:c2.fields |  fi in m1.b && fi.idField = f.id && c2 in c1.^extend && f.

 
}

pred noCalltoUndefinedField {
 all mi:FieldInvocation | some c1,c2: Class, m1:c1.methods, f:c2.fields |
     ( mi.qField = this_) => 
           mi in m1.b && mi.idField = f.id && ((c1 = c2) || ((c2 in c1.^extend) && (f.acc != private_)))
