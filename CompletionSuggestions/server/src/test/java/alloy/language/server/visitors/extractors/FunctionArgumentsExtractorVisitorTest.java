...
class FunctionArgumentsExtractorVisitorTest extends BaseVisitorTest {

	@Test
	public void testSimpleQuantifier() {
		CompletionModelBuilder modelBuilder = CourseModel.modelBuilder();
		String model =
				modelBuilder.withContent("fun f1[p: Person] : Person {").withCompletionLine(" some p ").withContent("}").build();
		var parser = buildParser(model);
		CompletionParams completionParams = buildCompletionParams(modelBuilder);

		FunctionArgumentsExtractorVisitor visitor = new FunctionArgumentsExtractorVisitor(model, completionParams);
