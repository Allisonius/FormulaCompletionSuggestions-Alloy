...
import arepair.generator.util.rules.difference.*;
import arepair.generator.util.rules.intersect.*;
import arepair.generator.util.rules.join.*;
import arepair.generator.util.rules.rclosure.RRule1;
import arepair.generator.util.rules.rclosure.RRule2;
import arepair.generator.util.rules.transpose.*;
import arepair.generator.util.rules.union.*;
...
public class Util {
    ...
    public static Card findCardinality(Sig sig) {
        if (sig.isLone != null) {
            return Card.LONE;
        }
        if (sig.isOne != null) {
...
//    }
    ...
    public static Expression createExprFromType(TypeInfo typeInfo, Map<String, String> uid) {
        if (SPECIAL_TYPE_TO_EXPRESSIONS.containsKey(typeInfo)) {
            return SPECIAL_TYPE_TO_EXPRESSIONS.get(typeInfo);
        }
        return initExprFromType(typeInfo, typeInfo.getUid());
    }

    public static Expression createExprFromType(TypeInfo typeInfo) {
        if (SPECIAL_TYPE_TO_EXPRESSIONS.containsKey(typeInfo)) {
            return SPECIAL_TYPE_TO_EXPRESSIONS.get(typeInfo);
        }
        return initExprFromType(typeInfo, typeInfo.getUid());
